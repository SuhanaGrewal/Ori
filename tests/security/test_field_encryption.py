...
from ori.security.field_encryption import decrypt_field, derive_or_load_key, encrypt_field
...
def test_derive_or_load_key_with_passphrase_creates_a_salt_file(tmp_path, monkeypatch):
    monkeypatch.setenv("ORI_ENCRYPTION_PASSPHRASE", "correct horse battery staple")
...
def test_derive_or_load_key_different_passphrases_produce_different_keys(tmp_path, monkeypatch):
    monkeypatch.setenv("ORI_ENCRYPTION_PASSPHRASE", "passphrase-one")
    key_one = derive_or_load_key(tmp_path / "a")

    monkeypatch.setenv("ORI_ENCRYPTION_PASSPHRASE", "passphrase-two")
