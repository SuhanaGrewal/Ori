...
from ori.inbox_intelligence.commitments import scan_for_commitments
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Ori inbox intelligence utilities.")
    ...
    config = load_config()
    ensure_dirs(config)
    logger = get_logger("ori.inbox_intelligence.cli", log_dir=config.log_dir)
    ...
    if args.command == "stale-threads":
        ...
        if account_email is None:
            print("Account email not captured yet - run `python -m ori.ingestion.gmail` first.")
            return
