...
from ori.common.logging import register_secret
...
def load_config(*, load_env_file=True) -> Config:
    ...
    notes_folder = os.environ.get("ORI_NOTES_FOLDER", "").strip()

    config = Config(
        data_dir=Path(os.environ.get("ORI_DATA_DIR", "./data")).expanduser().resolve(),
        log_dir=Path(os.environ.get("ORI_LOG_DIR", "./logs")).expanduser().resolve(),
