...
from ori.query.date_range import extract_date_range, parse_stored_date
from ori.query.router_prompt import (
    ...
    build_reply_status_target_message,
...
Intent = Literal[
    ...
    "calendar_conflicts", "reply_status", "general",
]

# how many of the account owner's own most-recently-sent threads to offer
# the reply-status matcher as candidates - wide enough to cover "a few
# months back" without dumping the entire sent history into one LLM call.
_REPLY_STATUS_CANDIDATE_LIMIT = 200
...
def _check_reply_status(
    ...
    *,
    ...
    """deterministic yes/no: did anyone other than the account owner ever
    send a message in the matched thread. Matching which thread the user
    means reuses the same "numbered candidates, one LLM call picks the
    index" pattern as _handle_draft_reply/_resolve_matching_item, over
    the account owner's own sent threads (list_threads_sent_by) - this
    only needs to identify WHICH thread, not judge its content, so it
    doesn't need semantic retrieval (route() has no embedder/reranker
    wired up, deliberately - see module docstring). Once matched, the
    actual answer is computed directly from the thread's own messages,
    same "deterministic over LLM" philosophy as _check_calendar_conflicts:
    a reply either exists or it doesn't, no judgment call needed. This
    also sidesteps the exact failure mode that motivated it - a reply-less
    thread's only message can have almost no body text of its own (e.g.
    an email whose real content is an attachment), which starves semantic
    retrieval's confidence scoring of anything to match against even
    though "has this been replied to" never needed that content at all."""
    sent = gmail_store.list_threads_sent_by(account_email, limit=_REPLY_STATUS_CANDIDATE_LIMIT)
    if not sent:
        return "You don't have any sent emails to check yet."

    labels = [f"'{row['subject'] or '(no subject)'}' sent {(row['sent_at'] or '')[:10]}" for row in sent]
    user_message = build_reply_status_target_message(text, labels)
    response = _call_llm(
        client=client, model=model, system=MATCH_REPLY_STATUS_TARGET_SYSTEM_PROMPT, text=user_message,
        analyzer=analyzer, max_tokens=20, logger=logger, audit_log_dir=audit_log_dir,
        operation="query.router_reply_status_match",
    )
    normalized = response.strip().upper()
    if "NONE" in normalized or not normalized.isdigit():
        return "I'm not sure which email you mean - could you be more specific?"

    index = int(normalized) - 1
    if not (0 <= index < len(sent)):
        return "I'm not sure which email you mean - could you be more specific?"

    thread = sent[index]
    subject = thread["subject"] or "(no subject)"
    sent_date = (thread["sent_at"] or "")[:10]

    account_email_lower = account_email.strip().lower()
    messages = gmail_store.list_messages_for_thread(thread["thread_id"])
    replies = [m for m in messages if parseaddr(m["sender"] or "")[1].lower() != account_email_lower]

    if not replies:
        return f"No reply yet to \"{subject}\" (you sent it {sent_date})."

    first_reply = replies[0]
    reply_date = (first_reply["sent_at"] or "")[:10]
    snippet = (first_reply["body_text"] or "").strip()
    if snippet:
        return f"Yes - {first_reply['sender']} replied on {reply_date}: {snippet[:280]}"
    return (
        f"Yes - {first_reply['sender']} replied on {reply_date} to \"{subject}\", but the reply "
        "itself has no readable text (likely an attachment-only message)."
    )
...
def route(
    ...
    *,
    ...
    if intent in ("stale_threads", "resolve", "draft_reply", "reply_status") and account_email is None:
        return RouterResult(
            intent=intent,
            answer="Account email not captured yet - run `python -m ori.ingestion.gmail` first.",
        )
    ...
    if intent == "reply_status":
        answer = _check_reply_status(
            text, gmail_store=gmail_store, account_email=account_email,
            client=client, model=model, analyzer=analyzer, logger=logger, audit_log_dir=audit_log_dir,
        )
        return RouterResult(intent="reply_status", answer=answer)
