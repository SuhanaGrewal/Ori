...
from ori.security.field_encryption import derive_or_load_key
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Generate and review Ori's periodic digest."
    )
    ...
    config = load_config()
    ensure_dirs(config)
    logger = get_logger("ori.digest.cli", log_dir=config.log_dir)
