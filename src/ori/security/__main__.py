...
from ori.redaction.entities import HARD_SECRET_ENTITIES
from ori.redaction.tokenize import tokenize_for_external_call, untokenize
from ori.security.audit_log import verify_audit_log


def main() -> None:
    parser = argparse.ArgumentParser(description="Ori security utilities.")
    ...
    if args.command == "verify-audit":
        ...
        if not rows:
            print("No Gmail messages with body text found - run `python -m ori.ingestion.gmail` first.")
            return
