...
from ori.webchat.digest_summary import build_morning_digest
from ori.webchat.initial_sync import run_initial_sync
from ori.webchat.titling import generate_card_title
...
# two different origins (frontend vs this API, whether that's two
# localhost ports in dev or tryori.vercel.app vs a Railway URL in prod)
# are two different origins to the browser - session state is therefore
# carried as a bearer token the frontend stores itself (matching the
# existing mock's own localStorage-based session, just storing a real
# opaque token instead of a raw user id), not a cookie. A cookie set by
# this server's origin would never reach fetch() calls made from the
# frontend's origin without extra cross-origin cookie plumbing (SameSite/
# proxy config) that isn't worth it here.
#
# both default to localhost for local dev; set ORI_FRONTEND_ORIGIN and
# ORI_BACKEND_BASE_URL in the deployed environment (e.g. Railway) to the
# real https:// origins. BACKEND_BASE_URL must match whatever redirect
# URI is registered for this OAuth client in Google Cloud Console.
...
class QueryBody(BaseModel):
    ...
    # set by the frontend only when filing a brand-new top-level card
    # (never a nested follow-up) - see titling.py's own docstring for why
    # that split matters.
...
def query(body: QueryBody) -> dict[str, Any]:
    ...
    if _client is not None and "gmail" not in excluded_sources:
        ...
        if router_result.answer is not None:
            ...
            return _with_title(
                {"id": answer_id, "question": body.question, "answer": router_result.answer, "citations": router_citations},
                body, per_user_config,
            )
    ...
    if result.abstained:
        ...
        return _with_title(
            {"id": answer_id, "question": body.question, "answer": answer, "citations": []},
            body, per_user_config,
        )
    ...
    return _with_title(
        {"id": answer_id, "question": body.question, "answer": answer_text, "citations": citations},
        body, per_user_config,
    )


def _with_title(response: dict[str, Any], body: QueryBody, per_user_config: Any) -> dict[str, Any]:
    """attaches a ChatGPT/Claude-style generated title to a query
    response, only when the frontend asked for one (generate_title=True
    - set for a brand-new card, never a follow-up) and an LLM is actually
    configured. A titling failure returns the response untouched (no
    "title" key) rather than raising - the frontend already treats a
    missing title as "fall back to the raw question," so this must never
    be allowed to break the answer itself."""
    if not body.generate_title or _client is None:
        return response
    title = generate_card_title(
        body.question, response["answer"], client=_client, model=per_user_config.llm_model, analyzer=_analyzer,
        logger=_logger, audit_log_dir=per_user_config.log_dir,
    )
    if title:
...
def _parse_window_param(value: str | None) -> datetime | None:
    """a caller-supplied since/until on the digest endpoint - the
    frontend's time-range picker. A bare (no-offset) timestamp, e.g. from
    an <input type="datetime-local"> with no timezone concept of its own,
    is treated as this machine's local time and localized rather than
    left naive - naive and aware datetimes can't be subtracted from each
    other, and this project already runs entirely on one person's own
    machine (see CLAUDE.md's local-first principle), so "local time" has
    one unambiguous meaning here, not a multi-user-across-timezones one."""
    if not value:
        return None
...
def digest(
    ...
    digest_data = build_morning_digest(
        stores["gmail_store"], stores["calendar_store"], stores["docs_store"], stores["notes_store"],
        stores["entity_store"], stores["inbox_store"], stores["gmail_store"].get_account_email(),
        client=_client, model=per_user_config.llm_model, analyzer=_analyzer,
        now=now, since=_parse_window_param(since), until=_parse_window_param(until),
        logger=_logger, audit_log_dir=per_user_config.log_dir,
        excluded_sources=revoked_sources(user),
    )
    return {"date": now.strftime("%a %b %d %Y"), **digest_data}
