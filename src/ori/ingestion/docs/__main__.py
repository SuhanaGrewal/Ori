...
from ori.ingestion.docs.sync import run_sync


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Sync Google Docs into Ori's local store (readonly)."
    )
    parser.add_argument(
        "--full-resync",
        action="store_true",
    ...
    config = load_config()
    ensure_dirs(config)
    logger = get_logger("ori.ingestion.docs.cli", log_dir=config.log_dir)
