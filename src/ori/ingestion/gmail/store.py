...
class GmailStore:
    ...
    def list_messages_for_thread(self, thread_id: str) -> list[sqlite3.Row]:
        """every non-deleted message in one thread, oldest first - the
        basis for a deterministic "has this been replied to yet" check
        (was every message in the thread sent by the account owner, or
        did someone else ever send one) rather than guessing from a
        single message's own (often near-empty, attachment-only) body
        text."""
        return self._conn.execute(
            "SELECT * FROM messages WHERE thread_id = ? AND is_deleted = 0 ORDER BY sent_at",
            (thread_id,),
        ).fetchall()

    def list_threads_sent_by(self, account_email: str, *, limit: int = 200) -> list[sqlite3.Row]:
        """one row per thread the account owner sent at least one message
        in - the message shown for each thread is the owner's OWN most
        recent message in it (not necessarily the thread's overall latest,
        which could be a later reply from someone else) - this is the
        candidate pool for "did X reply to my email about Y", so what
        matters for matching is what the user themselves sent and when,
        not whatever came after it. Most-recent-sent-first, capped at
        `limit` since a real mailbox can have thousands of threads and
        this feeds a single LLM disambiguation call, not a full scan."""
        ...
        rows = self._conn.execute(
            ...
            INNER JOIN (
                ...
                FROM messages
                WHERE is_deleted = 0 AND lower(sender) LIKE '%' || ? || '%'
                GROUP BY thread_id
            ) mine ON m.thread_id = mine.thread_id AND m.sent_at = mine.max_sent_at
            WHERE m.is_deleted = 0
            ORDER BY m.sent_at DESC
            LIMIT ?
            """,
            (account_email_lower, limit),
        ).fetchall()
        return rows
