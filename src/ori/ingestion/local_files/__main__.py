...
from ori.ingestion.local_files.scan import run_scan
from ori.ingestion.local_files.store import NotesStore


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Scan the local notes/transcripts folder into Ori's local store."
    )
    parser.add_argument(
        "--force-rehash",
        action="store_true",
    ...
    config = load_config()
    ensure_dirs(config)
    logger = get_logger("ori.ingestion.local_files.cli", log_dir=config.log_dir)

    if config.notes_folder is None:
        print("ORI_NOTES_FOLDER is not set. Set it in .env and try again.")
        sys.exit(1)
